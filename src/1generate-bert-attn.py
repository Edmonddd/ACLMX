...
parser.add_argument('-dir',default="../data/100",  type=str,  help="directory to save everything")
parser.add_argument('-ckpt',default="../data/100/zh_flair_xlm_100/best-model.pt",  type=str,  help="model file path")
parser.add_argument('-tf',default="../data/100/zh_sample_train.conll",  type=str,  help="training file path")
parser.add_argument('-df',default="../data/100/zh_dev.conll",  type=str,  help="dev file path")
...
# model = AutoModel.from_pretrained("/home/lxm/ACLM-main/xlm-roberta-large",output_hidden_states=True, output_attentions=True)
model = AutoModel.from_pretrained("M:\llc\\fewshotLearning\\few\\read\promptlearning\\3\\1ACLM A Selective-Denoising based Generative Data Augmentation\\ACLM-main\\xlm-roberta-large",output_hidden_states=True, output_attentions=True)
# tokenizer = AutoTokenizer.from_pretrained("/home/lxm/ACLM-main/xlm-roberta-large", add_prefix_space=True)
tokenizer = AutoTokenizer.from_pretrained("M:\llc\\fewshotLearning\\few\\read\promptlearning\\3\\1ACLM A Selective-Denoising based Generative Data Augmentation\\ACLM-main\\xlm-roberta-large", add_prefix_space=True)
bert_keys = list(model.state_dict().keys())
print(len(bert_keys)) # bert_keys的数量391
...
# 尝试加载模型参数的状态字典，同时在参数不匹配的情况下进行灵活处理，以确保模型能够正确加载。异常处理块中的代码在加载失败时提供了备选的加载方式。
...
def split_at_punctuation(s):
    delimiters = re.findall(r',', s) # 查找字符串 s 中所有逗号 , 的位置，并将它们存储在 delimiters 列表中。这个步骤用于获取逗号的位置
    split_list = re.split(r',', s) # 按照逗号 , 对字符串 s 进行分割，并将分割后的部分存储在 split_list 列表中
    result = []
