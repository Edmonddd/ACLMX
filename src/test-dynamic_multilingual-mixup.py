...
model = SentenceTransformer("/home/hsq/ACLM-main/distiluse-base-multilingual-cased-v2")
...
generated_file = args.root_dir + "/" + args.input_file + '-' + args.file_name + '-mixup' + '.txt'
...
if args.sample_generation_mode=='static':
    ...
    with open(generated_file, 'a') as the_file:
        ...
        for i in tqdm(range(len(text))):
            saved = {}
            for z in range(int(args.num_of_sequences)-2):
                new_text = text[i].split()
                new_label = label[i].split()
                new_bert_attn = bert_att[i].split()
