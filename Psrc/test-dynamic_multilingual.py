...
parser = argparse.ArgumentParser(allow_abbrev=False)
parser.add_argument('--model', default="/home/lxm/ACLM-main/data/100/zh_sample_train_attn_0.3_xlm-roberta-large-0.3-false-gauss-attention-dynamic-0.3-5-false-100-xlm-large-zh-42-retrain-final",help='which model to use') 
parser.add_argument('--input_file','-i', default="zh_sample_train_attn_0.3_xlm-roberta-large",help='input file to use')
...
parser.add_argument('--root_dir','-ro', type=str, default="/home/lxm/ACLM-main/data/100", help='root directory')
...
newFileName = args.directory + args.input_file + '.csv' # input文件
...
def isGeneratedSentenceValid(sent):
    ...
    return True # 通过遍历生成的句子中，new_token的数量是否为偶数去判定生成的句子是否有效。
...
# DYNAMIC MASKING NEW CODE 动态mask
    ...
    with open(generated_file, 'w') as the_file:
        ...
        for i in tqdm(range(len(text))):
            ...
            for z in range(int(args.num_of_sequences)):
                ...
                new_bert_attn = bert_att[i].split()
                if args.mode == 'attn':
                    new_sketch = mask_entities(new_text, new_label, False) # 对实体标签进行操作
                    new_sketch = mask_words(new_sketch, new_label, new_bert_attn, args.mask_attn) # 对句子进行mask操作
                elif args.mode == 'both':
                    new_sketch = mask_entities(new_text, new_label, args.mask_entities)
                    new_sketch = mask_words(new_sketch, new_label, new_bert_attn, args.mask_attn)
                elif args.mode == 'either':
                ...
                # print(generated_text[0]['generated_text']) 通过关键字和实体标签生成新的句子，赋值给 generated_text
                # continue
                if args.remove_repetitions:
                ...
                prev_label = ''
                temp = False
                for k in generated_text[0]['generated_text'].split(' '): # 通过索引 [0] 访问了 generated_text 列表的第一个元素，然后使用 ['generated_text'] 访问了这个元素中键为 'generated_text' 的值，即一段文本字符串。
                    if k=='':
                        continue
                    if prev_label=='' and k[0]!='<':
                        the_file.write(f'{k}\tO\n')
                        ...
                        prev_label = k[1:-1].upper() # 提取标签中<>内容
                        continue
                    else:
                        if temp:
                            the_file.write(f' {k}')
                ...
                the_file.write('\n') # 这段for循环将生成新的text写入到zh_sample_train_attn_0.3_xlm-roberta-large-0.3-false-gauss-attention-dynamic-0.3-5-false-100-xlm-large-zh-42-retrain.txt文件中，并且将其按照 [token label]进行写入
            # the_file.write('\n')
            # the_file.write('----\n\n')
